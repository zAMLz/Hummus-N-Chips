...
int main(int argc, char *argv[]) {
    ...
    if(!dot || (dot && strcmp(dot, ".hex"))) {
        fprintf(stderr, "%s is not a .hex file!", file_name);
        exit(EXIT_FAILURE);
    }

    return simulate_hummus(file_name_path);
}

// Prints the usage message to the STDERR
void usage_message(char* exec_name) {
    fprintf(stderr, "Usage %s [-h] [-@ flag ...] program.hex\n", exec_name);
}

// Prints the help message to the STDOUT
void help_message(char* exec_name) {
    printf("SYNOPSIS\n");
    printf("    %s [-h] [-@ flag ...] program.hex\n", exec_name);
    printf("\n");
    printf("OPTIONS\n");
